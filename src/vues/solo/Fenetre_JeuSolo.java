...
KeyListener
{
	 ...
	 */  
	private static JEditorPane taConsole  = new JEditorPane("text/html","");
	 ...
	 */
	public static void ajouterTexteHTMLDansConsole(String texte)
	{
		String s = taConsole.getText();
		int endBody = s.indexOf("</body>");
		if(firstWriteToConsole){
			taConsole.setText( s.substring(0,s.indexOf("<body>")) 
					+ texte + 
					s.substring(endBody));
			firstWriteToConsole = false;
		}
		else{
			taConsole.setText( s.substring(0,endBody) 
					+ texte + 
					s.substring(endBody));
		}
