 ...
 * <p>
 * Ce panel affiche la zone de jeu avec tous les elements que contient le
 * terrain.
 * <p>
 ...
 */
public class Panel_Terrain extends JPanel
		implements Runnable, MouseListener, MouseMotionListener, KeyListener, MouseWheelListener {
	private static final long serialVersionUID = -7842146139874652019L;

	// -------------------------
	// -- proprietes du panel --
	// -------------------------
	private static final int xOffsetPseudo = 10, yOffsetPseudo = 30;
	...
	/**
	 * Coefficient de largeur de la barre de vie d'une creature. (en % de la
	 * largeur de la creature)
	 */
	 ...
	 */
	protected int decaleX = 0, decaleY = 0;

	/**
	 * Marges internes du chateau pour coloration du centre (couleur de
	 * l'équipe)
	 */
	...
	private static final BasicStroke TRAIT_TILLE = new BasicStroke(1.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER,
			10.0F, DASHES, 0.F);

	private static final float[] DASHES_EPAIS = { 8.0F, 4.0F };
	private static final BasicStroke TRAIT_TILLE_EPAIS = new BasicStroke(4.0f, BasicStroke.CAP_ROUND,
			BasicStroke.JOIN_ROUND, 10.0F, DASHES_EPAIS, 0.F);
	...
	/**
	 * Marge autour du terrain pour éviter des bugs de déplacements en dehors
	...
	/**
	 * Permet de savoir le decalage effectuer depuis le position du debut de
	 * l'agrippage
	 */
	...
	/**
	 * Le terrain permet de choisir la tour a poser sur le terrain. Si cette
	 * variable est non nulle et que le joueur clique sur le terrain, la tour a
	 * ajouter sera posée.
	 */
	private Tour tourAAjouter;

	/**
	 * Lorsque le joueur clique sur une tour, elle devient selectionnee. Une
	 * fois selectionnee des informations sur la tour apparaissent dans le menu
	 * d'interaction. Le joueur pourra alors améliorer ou vendre la tour.
	 */
	private Tour tourSelectionnee;

	/**
	 * Lorsque le joueur clique sur une creature, elle devient selectionnee. Une
	 * fois selectionnee des informations sur la creature apparaissent dans le
	 * menu d'interaction.
	 */
	...
	/**
	 * Permet de répéter l'image de fin de tel sorte à recouvrir toute la
	 * surface du terrain
	 */
	...
	static {
		/*
		 * TODO [INFO] curseur transparent int[] pixels = new int[16 * 16];
		 * Image image = Toolkit.getDefaultToolkit().createImage( new
		 * MemoryImageSource(16, 16, pixels, 0, 16)); curTransparent =
		...
		curMainAgripper = Toolkit.getDefaultToolkit().createCustomCursor(iHandGrab, new Point(0, 0), "grab");
	}
...
//		setPreferredSize(jeu.getTerrain().getTaillePanelTerrain());
	...
	/**
	 * Permet de basculer de l'affichage au non affichage du maillage et vis
	 * versa.
	 * 
	...
	public void paintComponent(Graphics g) {
		 ...
		 * RenderingHints.VALUE_RENDER_Constants);
		 * g2.setRenderingHint(RenderingHints.KEY_COLOR_RENDERING,
		 * RenderingHints.VALUE_COLOR_RENDER_Constants);
		...
		g2.fillRect(-MARGE_UNIVERS, -MARGE_UNIVERS, jeu.getTerrain().getLargeur() + 2 * MARGE_UNIVERS,
		...
		for (Equipe equipe : jeu.getEquipes()) {
			...
			if (equipe.getZoneArriveeCreatures() != null) {
				...
				if (modeDebug) {
					...
					// sol de couleur
					g2.setColor(equipe.getCouleur());
					g2.fillRect(r.x + MARGES_CHATEAU, r.y + MARGES_CHATEAU, r.width - (2 * MARGES_CHATEAU),
							r.height - (2 * MARGES_CHATEAU));
		...
		if (afficherMaillage) {
			...
			if (arcsActifs != null) {
				...
				for (Line2D arc : arcsActifs)
					g2.drawLine((int) arc.getX1(), (int) arc.getY1(), (int) arc.getX2(), (int) arc.getY2());
			}
			...
			/*
			 * TODO Affichage de tous les chemins (pour maillage v2.0) try {
			 * g2.setColor(Color.BLUE); for(int i=-5;i<50;i++) for(int
			 * j=-5;j<50;j++) afficherCheminPourNoeud(i*10, j*10,g2);
			 * 
			 * 
		...
		if (tourSelectionnee != null) {
			...
			g2.drawRect(tourSelectionnee.getXi(), tourSelectionnee.getYi(), (int) (tourSelectionnee.getWidth()),
		...
		if (creatureSelectionnee != null) {
			g2.setColor(COULEUR_SELECTION);
			g2.setStroke(TRAIT_TILLE);
			g2.drawOval((int) (creatureSelectionnee.getX()), (int) (creatureSelectionnee.getY()),
					(int) creatureSelectionnee.getWidth(), (int) creatureSelectionnee.getHeight());
			...
			if (centrerSurCreatureSelectionnee)
				centrerSur((int) creatureSelectionnee.getCenterX(), (int) creatureSelectionnee.getCenterY());
	 ...
	 * 
	 * try { chemin = jeu.getTerrain().getCheminLePlusCourt(x, y, 0, 0, 0);
	 * 
	 * Point pPrec = chemin.get(0); for(int i=1;i<chemin.size();i++) { if(i ==
	 * 2) break;
	 * 
	 * Point p = chemin.get(i);
	 * 
	 * g2.drawLine((int) pPrec.x, (int) pPrec.y, (int) p.x, (int) p.y);
	 * 
	 * pPrec = p; } } catch (IllegalArgumentException e){} catch
	 ...
	 * @param g2
	 *            le Graphics2D pour dessiner
	 */
	private void dessinerZone(final Rectangle zone, final Graphics2D g2) {
		g2.fillRect((int) zone.getX(), (int) zone.getY(), (int) zone.getWidth(), (int) zone.getHeight());
	}
	...
	private void dessinerCreature(final Creature creature, final Graphics2D g2) {
		if (creature.getImage() != null) {
			...
			tx.scale(creature.getWidth() / (double) creature.getImage().getWidth(null),
					creature.getHeight() / (double) creature.getImage().getHeight(null));
			...
			g2.fillOval((int) creature.getX(), (int) creature.getY(), (int) creature.getWidth(),
	...
	private void dessinerBarreDeSante(final Creature creature, final Graphics2D g2) {
		...
		int positionXBarre = (int) (creature.getX() - (largeurBarre - creature.getWidth()) / 2);
		int positionYBarre = (int) (creature.getY() + creature.getHeight());

		// affichage du conteneur
		g2.setColor(creature.getProprietaire().getEquipe().getCouleur());

		g2.fillRect(positionXBarre, positionYBarre, largeurBarre, HAUTEUR_BARRE_VIE);

		// affichage du contenu
		g2.setColor(COULEUR_SANTE);

		g2.fillRect(positionXBarre + 1, positionYBarre + 1,
				(int) (creature.getSante() * (largeurBarre - 2) / creature.getSanteMax()), HAUTEUR_BARRE_VIE - 2);
	}
		...
		if (chemin != null && chemin.size() > 0) {
			...
			synchronized (chemin) {
				...
				for (int i = creature.getIndiceCourantChemin(); i < chemin.size(); i++) {
					/*
					 * affichage du segment de parcours entre le point precedent
					 * et la suivant
					 */
					point = chemin.get(i);
	 ...
	 *            le Graphics2D pour dessiner
	 * @param avecPortee
	 *            dessiner ou non la portee de la tour ?
	 */
	private void dessinerTour(final Tour tour, final Graphics2D g2, final boolean avecPortee) {
		// dessin de l'image
		...
		// dessin d'un forme de couleur
			...
			g2.fillOval(tour.getXi(), tour.getYi(), (int) tour.getWidth(), (int) tour.getHeight());
		}
	...
	private void dessinerPortee(Tour tour, Graphics2D g2, Color couleurRayonDePortee) {
		...
		g2.drawOval((int) (tour.getXi() - tour.getRayonPortee() + tour.getWidth() / 2),
				(int) (tour.getYi() - tour.getRayonPortee() + tour.getHeight() / 2), (int) tour.getRayonPortee() * 2,
	 ...
	 * 
	 * @param tauxTransparence
	 *            le taux (1.f = 100% opaque et 0.f = 100% transparent)
	 * @param g2
	 *            le Graphics2D a configurer
	 */
	 ...
	 * 
	 * @param me
	 *            l'evenement lie a cette action
	 * @see MouseListener
	 */
	@Override
		...
		if (me.getButton() == MouseEvent.BUTTON1) {
			if (me.getClickCount() == 1) {
				...
				for (Tour tour : jeu.getTours()) // pour chaque tour...
					if (tour.intersects(positionSurTerrain.x, positionSurTerrain.y, 1, 1)) // la
																							// souris
																							// est
																							// dedans
																							// ?
				...
				// parcours a l'envers car il faut traiter les creatures les
				// plus
				// devant en premier (les derniers affiches)
				for (int i = creatures.size() - 1; i >= 0; i--) {
					creature = creatures.get(i);

					if (creature.intersects(positionSurTerrain.x, positionSurTerrain.y, 1, 1)) // la
																								// souris
																								// est
																								// dedans
																								// ?
					{
						...
						// si le joueur clique sur une creature deja
						// selectionnee
						if (creatureSelectionnee == creature)
							creatureSelectionnee = null; // deselection

						else
							creatureSelectionnee = creature; // la creature est
																// selectionnee
				...
				// aucune tour et aucune creature trouvee => clique dans le
				// vide.
	...
	public void mouseReleased(MouseEvent me) {
		if (!jeu.estEnPause()) {
			...
			if (tourAAjouter != null) {
				...
				// informe la fenetre qu'une tour a ete selectionnee
				// edpt.tourSelectionnee(tourAAjouter,
				// Panel_InfoTour.MODE_SELECTION);
			}
		}
	}
	...
	public void mouseMoved(MouseEvent me) {
		...
		if (tourAAjouter != null) {
			...
			tourAAjouter.setLocation(getPositionSurQuadrillage(p.x), getPositionSurQuadrillage(p.y));
		}
	}
	...
	/**
	 * Permet de faire correspondre une coordonnée donnée sur la position
	 * normale du terrain (sans zoom et décalage)
	 * 
	 * @param x
	...
	protected Point getCoordoneeSurTerrainOriginal(int x, int y) {
		return new Point((int) (x / coeffTaille - decaleX), (int) (y / coeffTaille - decaleY));
	}
	 ...
	 * 
	 * @param p
	 *            la coordonnnée donnée
	 * @return le point correspondant sur le terrain de taille normal
	 */
	protected Point getCoordoneeSurTerrainOriginal(Point p) {
		return new Point((int) (p.x / coeffTaille - decaleX), (int) (p.y / coeffTaille - decaleY));
	}
	...
	public void mouseDragged(MouseEvent me) {
		if (boutonGrab == MouseEvent.BUTTON1) {
			// si il n'y a pas de tour a ajouter, c'est comme si elle bougeait
			// normalement
			if (tourAAjouter != null)
				mouseMoved(me);
	 ...
	 * 
	 * @param x
	 *            la position sur le terrain normal
	 * @param y
	 *            la position sur le terrain normal
	 */
	...
	public void reinitialiserVue() {
		// remise à l'échelle initiale et recentrage
		// coeffTaille = 1.0;
		// centrerSur(jeu.getTerrain().getLargeur()/2,
		// jeu.getTerrain().getHauteur()/2);
		center();
	}
	...
	public void zoomer(int i) {
		...
		if (sourisSurTerrain)
			...
			pAvant = new Point((int) getPreferredSize().getWidth() / 2, (int) getPreferredSize().getHeight() / 2);
		...
		if (coeffTaille < ZOOM_MIN) {
			...
			if (sourisSurTerrain) {
				...
				centrer(pAvant.x, pAvant.y);
		}

		/*
		 * if(!sourisSurTerrain) // centrer sur le milieu du terrain
		 * centrerSur(jeu.getTerrain().getLargeur()/2,
		 * jeu.getTerrain().getHauteur()/2);
		 */
	}
	...
	public void center() {
		// remise à l'échelle initiale et recentrage
		coeffTaille = 1.5;
		centrer(jeu.getTerrain().getLargeur() / 2, jeu.getTerrain().getHauteur() / 2);
	}

	private void centrer(int x, int y) {
		setPreferredSize(new Dimension(700, 750));

		int largeurPanel = getPreferredSize().width;
